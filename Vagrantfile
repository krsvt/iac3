Vagrant.configure("2") do |config|
  ...
  def create_host(config, hostname, ip, host_port)
    config.vm.define hostname do |host|
      ...
      host.vm.network "forwarded_port", guest: 8000, host: host_port
      yield host if block_given?
    end
  end

  (1..SERVERS).each do |machine_id|
    create_host(config, "srv#{machine_id}", "192.168.56.#{200+machine_id}", 8000+machine_id)
  end
